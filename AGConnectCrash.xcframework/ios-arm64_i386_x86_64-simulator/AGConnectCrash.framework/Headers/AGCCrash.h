 ...
 * @param exception  自定义的异常。
 */
- (void)recordFatalExceptionModel:(AGCExceptionModel *)exception;

/**
 * 是否立即上传崩溃
 * @param enable YES立即上传 NO下次上传 默认是YES。
 */
- (void)enableUploadCrashImmediately:(BOOL)enable NS_SWIFT_NAME(enableUploadCrashImmediately(enable:));
